...
import { managerAPI, categoryAPI } from "../../api/index";
...
const PRICE_FILTERS = [
  ...
  { label: "300K-500K", value: "300-500" }, { label: "500K-1M", value: "500-1000" }, { label: "> 1M", value: "1000+" },
];
const SORT_OPTIONS = [
  { label: "Default", value: "default" }, { label: "Price ↑", value: "price-asc" },
  { label: "Price ↓", value: "price-desc" }, { label: "Name A-Z", value: "name-asc" },
  { label: "Name Z-A", value: "name-desc" }, { label: "Stock ↑", value: "stock-asc" }, { label: "Stock ↓", value: "stock-desc" },
];
...
export default function ManagerProducts() {
  ...
  const fetchProducts = async () => {
    try { setLoading(true); const r = await managerAPI.getProducts(); setProducts(r.data?.products || r.products || r.data || r || []); }
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try { const r = await managerAPI.getProducts(); setProducts(r.data?.products || r.products || r.data || r || []); } catch {}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingProduct) { await managerAPI.updateProduct(editingProduct._id, data); Alert.alert("Success", "Product updated!"); }
      else { await managerAPI.createProduct(data); Alert.alert("Success", "Product added!"); }
  ...
  const handleDelete = (id: string) => {
    Alert.alert("Confirm", "Delete this product?", [
      ...
      { text: "Delete", style: "destructive", onPress: async () => {
        try { await managerAPI.deleteProduct(id); Alert.alert("Success", "Deleted!"); fetchProducts(); }
      ...
      <View style={st.cardRow}>
        ...
        <View style={{ flex: 1, marginLeft: 12 }}>
          ...
          <View style={{ flexDirection: "row", gap: 12, marginTop: 4 }}>
            <View style={st.metaItem}><FontAwesome5 name="box" size={10} color={C.light} /><Text style={st.prodMeta}>{item.stock}</Text></View>
            <View style={st.metaItem}><FontAwesome5 name="tag" size={10} color={C.light} /><Text style={st.prodMeta}>{item.category?.name || "N/A"}</Text></View>
          </View>
        </View>
      </View>
      <View style={st.actions}>
        <TouchableOpacity style={[st.actionBtn, { borderColor: C.blue }]} onPress={() => openEdit(item)}>
          <FontAwesome5 name="edit" size={13} color={C.blue} /><Text style={[st.actionBtnText, { color: C.blue }]}>Edit</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[st.actionBtn, { borderColor: C.red }]} onPress={() => handleDelete(item._id)}>
          <FontAwesome5 name="trash-alt" size={13} color={C.red} /><Text style={[st.actionBtnText, { color: C.red }]}>Delete</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
    ...
    <View style={st.container}>
      ...
      <View style={st.filterRow}>
        <TouchableOpacity style={st.chip} onPress={() => setShowFilterMenu(showFilterMenu === "price" ? null : "price")}>
          <FontAwesome5 name="dollar-sign" size={11} color={C.text} style={{ marginRight: 6 }} /><Text style={st.chipText}>Price</Text>
        </TouchableOpacity>
        <TouchableOpacity style={st.chip} onPress={() => setShowFilterMenu(showFilterMenu === "cat" ? null : "cat")}>
          <FontAwesome5 name="tag" size={11} color={C.text} style={{ marginRight: 6 }} /><Text style={st.chipText}>Category</Text>
        </TouchableOpacity>
        <TouchableOpacity style={st.chip} onPress={() => setShowFilterMenu(showFilterMenu === "sort" ? null : "sort")}>
          <FontAwesome5 name="sort" size={11} color={C.text} style={{ marginRight: 6 }} /><Text style={st.chipText}>Sort</Text>
        </TouchableOpacity>
      </View>
      ...
      <TouchableOpacity style={st.addBtn} onPress={openAdd}>
        <FontAwesome5 name="plus" size={12} color={C.white} /><Text style={st.addText}>Add Product</Text>
      </TouchableOpacity>
      {loading ? (
            ...
            <ScrollView>
              ...
              <View style={st.field}><Text style={st.label}>Image URLs</Text>
                ...
                <TouchableOpacity style={st.addImgBtn} onPress={() => setFormData(p => ({ ...p, images: [...p.images, ""] }))}>
                  <Text style={{ color: C.primary, fontWeight: "600" }}>+ Add Image</Text>
                </TouchableOpacity>
              </View>
...
const st = StyleSheet.create({
  ...
  chip: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: C.white, borderRadius: 8, paddingVertical: 10, borderWidth: 1, borderColor: C.border },
  chipText: { fontSize: 13, color: C.text, fontWeight: "500" },
  dropdown: { backgroundColor: C.white, marginHorizontal: 12, borderRadius: 8, borderWidth: 1, borderColor: C.border, marginTop: 4 },
  dropItem: { paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: C.border },
  dropText: { fontSize: 14, color: C.text },
  dropActive: { fontSize: 14, color: C.primary, fontWeight: "700" },
  addBtn: { backgroundColor: C.primary, marginHorizontal: 12, paddingVertical: 14, borderRadius: 8, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 12 },
  addText: { color: C.white, fontWeight: "700", fontSize: 14 },
  card: { backgroundColor: C.white, borderRadius: 12, padding: 14, marginBottom: 12, marginHorizontal: 12, elevation: 1 },
  cardRow: { flexDirection: "row" },
  thumb: { width: 70, height: 70, borderRadius: 10 },
  prodName: { fontSize: 16, fontWeight: "700", color: C.text },
  prodPrice: { fontSize: 15, fontWeight: "600", color: C.green, marginTop: 2 },
  metaItem: { flexDirection: "row", alignItems: "center", gap: 4 },
  prodMeta: { fontSize: 12, color: C.light },
  ...
  modalContent: { backgroundColor: C.white, borderRadius: 16, padding: 20, maxHeight: "90%" },
  modalTitle: { fontSize: 18, fontWeight: "700", color: C.text, marginBottom: 16 },
  field: { marginBottom: 14 },
  label: { fontSize: 13, fontWeight: "600", color: C.text, marginBottom: 6 },
  input: { borderWidth: 1, borderColor: C.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 12, fontSize: 14, color: C.text },
  chip2: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, backgroundColor: "#F3F4F6", marginRight: 8 },
  chip2Active: { backgroundColor: C.primary },
  chip2Text: { fontSize: 13, color: C.text },
  chip2TextActive: { fontSize: 13, color: C.white, fontWeight: "600" },
  sizeBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8, backgroundColor: "#F3F4F6" },
  sizeBtnActive: { backgroundColor: C.indigo },
  sizeBtnText: { fontSize: 13, color: C.text },
  sizeBtnTextActive: { fontSize: 13, color: C.white, fontWeight: "600" },
  removeImgBtn: { justifyContent: "center", paddingHorizontal: 8 },
  addImgBtn: { alignItems: "center", paddingVertical: 12, borderWidth: 1, borderStyle: "dashed", borderColor: C.border, borderRadius: 8, marginTop: 4 },
